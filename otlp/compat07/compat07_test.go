...
func TestAC1_1_IntGaugeConversion(t *testing.T) {
	...
	converted, err := DetectAndConvertMetrics(metrics)
	require.NoError(t, err)
...
func TestAC1_5_MalformedBytesError(t *testing.T) {
	...
	converted, err := DetectAndConvertMetrics([]*metricspb.Metric{m})
	assert.Error(t, err)
...
func TestAC4_1_MixedPayloads(t *testing.T) {
	...
	// Second metric should be DoubleGauge (1.x) with labels converted to attributes
	m1 := converted[1]
	...
	// Verify labels converted to attributes on 1.x-recognized metric
